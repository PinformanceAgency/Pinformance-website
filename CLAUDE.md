...
- **Organic — P3.3.7 seeding has no pacing at all, and its own comment used to claim it did.** It said it "paces itself at seedsPerMinute and stops as soon as it hits the DB's check_daily_volume ceiling". Neither is true: there is no delay between calls, no such option is read, and `check_daily_volume()` is a trigger on INSERTs into `organic.pins` — seeding writes none, it calls Pinterest directly and increments `boards.seeded_count`. A run over 23 boards at the default 15 per board is **up to 345 createPin calls as fast as the API allows**, which is the activity spike module 4 warns about, on an account that may be weeks old. The comment now says what the code does. The pace itself is left undecided on purpose: what a seeding burst may safely be is a number from the method, and one invented here would read as a real rule three months from now. Until that decision, seed in batches of boards and lower `seedsPerBoardMax`.
