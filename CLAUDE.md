...
- **Zones → Last month is the invoice basis.** `StoreZoneRow.last_month` carries the finished month's spend/revenue/ROAS/zone, the full-month scale floor, and the period the store actually ran that month. Three rules the tab does not break: amounts stay in each ad account's own currency and totals are grouped **per currency**, a store we hold no data for shows an em dash rather than €0, and a store offboarded since the month ended is still listed if it spent during that month — it gets invoiced for the month it ran. The month is located **by key** in `monthBucketKeys(end)`, never at bucket index 1: the zone window ends *yesterday*, so on the 1st and 2nd of a month the three buckets are the three months before it. Invoicing happens on the 3rd/4th, right next to that edge.
- **A short month is not a missing month — report the shape, not the day count.** Counting days cannot tell "offboarded on the 8th" apart from "we lost eight days of data", and the first is the ordinary case: a store is invoiced for the part of the month it ran. `last_month` therefore carries `measured_from` / `measured_through` (the period it ran) and `gap_days` (days with no snapshot *between* those two). Only `gap_days > 0` is a defect and gets a warning; a store starting or stopping mid-month is stated neutrally with its period. Measured on August 2026 this went from 11 alarming-looking stores to 3 real gaps and 8 ordinary on/offboardings — and surfaced Hudson Ridge, offboarded 08-06 but carrying data to 08-18 with 7 days missing in between, which the flat count had buried among the others. Verify with `DOTENV_CONFIG_PATH=.env.local npx tsx scripts/check-invoice-month.ts` (`MONTH=2026-07` for an older one), which compares every store's figures against a plain SQL SUM over the same rows and exits 1 on any difference.
