...
- **Weekly-update call budget — O(1) in stores, not O(n), in both crons**: reckon with **~60s of wall clock, not the declared `maxDuration = 300`**. Both runs were cut off around a minute on 17-08-2026: the seed after 18 of 49 stores (~55s), the sync after 13 of 37. The declared limit is not what applies in practice, so neither run may do per-store API calls in a sequential loop.
  ...
  - `weekly-update-sync` does the same: one board-wide read up front, handed to `writeWeek(..., preloaded)`, and the stores processed `SYNC_BATCH` (6) at a time. Dry run over 37 stores: ~30s. Don't raise the batch much higher — Monday throttles parallel mutations and the Pinterest reports have their own rate limit.
  ...
  - **A seed or sync log without a final `EINDCONTROLE` line means the run was cut off**, whatever the counters above it say. Note that a cut-off run also never reaches the alert, so Slack stays silent — absence of the line is the signal.
